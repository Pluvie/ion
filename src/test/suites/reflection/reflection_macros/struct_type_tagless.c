    ...
    struct reflection rfx = {
      type(STRUCT, struct example), fields({
        ...
        { field(inside_1, STRUCT, struct example), fields({
            { field(string, POINTER, struct example, inside_1),
                of({ type(CHAR) })
            },
            { field(number, U32, struct example, inside_1) },
            { field(inside_2, STRUCT, struct example, inside_1), fields({
                { field(string, POINTER, struct example, inside_1.inside_2),
                    of({ type(CHAR) })
                },
                { field(number, U32, struct example, inside_1.inside_2) },
              })
            },
          })
        },
