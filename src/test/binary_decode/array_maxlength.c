test( binary_decode, array_maxlength ) {
  ...
  when("it has an associated schema that enforces a maximum length")
    struct reflect schema = {
      type(STRUCT, sizeof(struct example)), fields({
        { field(struct example, bytes), type(ARRAY, 0, 2), of({ type(BYTE) }) },
      })
    };
  ...
  must("fail to decode the input data and report the error properly");
    ...
    verify(streq(error.message,
      "[bytes] array required maximum length of 2 but found 3"));
