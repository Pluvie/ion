test( binary_decode, vector ) {

  given("an example struct with a vector type");
    struct example {
      struct vector one_dim;
      struct vector two_dim;
      struct vector three_dim;
    } example;
  ...
  when("it has an associated reflection");
    ...
    struct reflect example_reflection = {
      type(STRUCT, sizeof(struct example)), fields({
        ...
        { field(struct example, one_dim), type(VECTOR), of({
            type(STRUCT), reflect(&element_reflection)
          })
        },

        { field(struct example, two_dim), type(VECTOR), of({
            type(VECTOR), of({
              type(STRUCT), reflect(&element_reflection)
            })
          })
        ...
        { field(struct example, three_dim), type(VECTOR), of({
            type(VECTOR), of({
              type(VECTOR), of({
                type(U8)
              })
  ...
  must("decode the input data on the struct correctly");
    ...
    struct vector* one_dim = &(example.one_dim);
    e = as(struct element, vector_get(one_dim, 0));
    verify(e.a == 0);
    verify(e.b == 1);
    e = as(struct element, vector_get(one_dim, 1));
    verify(e.a == 2);
    verify(e.b == 3);
    e = as(struct element, vector_get(one_dim, 2));
    verify(e.a == 4);
    verify(e.b == 5);

    struct vector* two_dim = &(example.two_dim);
    e = as(struct element, vector_get((struct vector*) vector_get(two_dim, 0), 0));
    verify(e.a == 16);
    verify(e.b == 17);
    e = as(struct element, vector_get((struct vector*) vector_get(two_dim, 0), 1));
    verify(e.a == 18);
    verify(e.b == 19);
    e = as(struct element, vector_get((struct vector*) vector_get(two_dim, 1), 0));
    verify(e.a == 32);
    verify(e.b == 33);
    e = as(struct element, vector_get((struct vector*) vector_get(two_dim, 1), 1));
    verify(e.a == 34);
    verify(e.b == 35);

    u8* v;
    struct vector* three_dim = &(example.three_dim);
    v = vector_get((struct vector*)
          vector_get((struct vector*)
            vector_get(three_dim, 0), 0), 0);
    verify(v[0] == 0);
    verify(v[1] == 1);
    v = vector_get((struct vector*)
          vector_get((struct vector*)
            vector_get(three_dim, 0), 1), 0);
    verify(v[0] == 16);
    verify(v[1] == 17);
    v = vector_get((struct vector*)
          vector_get((struct vector*)
            vector_get(three_dim, 1), 0), 0);
    verify(v[0] == 32);
    verify(v[1] == 33);
    v = vector_get((struct vector*)
          vector_get((struct vector*)
            vector_get(three_dim, 1), 1), 0);
    verify(v[0] == 48);
    verify(v[1] == 49);
    v = vector_get((struct vector*)
          vector_get((struct vector*)
            vector_get(three_dim, 2), 0), 0);
    verify(v[0] == 64);
    verify(v[1] == 65);
    v = vector_get((struct vector*)
          vector_get((struct vector*)
            vector_get(three_dim, 2), 1), 0);
    verify(v[0] == 80);
    verify(v[1] == 81);
