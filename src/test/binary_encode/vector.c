test( binary_encode, vector ) {
  ...
  when("some input data is ready to encode");
    ...
    input.one_dim = vector_of(struct element, {
      { .a = 0, .b = 1 },
      { .a = 2, .b = 3 },
      { .a = 4, .b = 5 },
    });
    input.two_dim = vector_of(struct vector, {
      vector_of(struct element, {
        { .a = 16, .b = 17 },
        { .a = 18, .b = 19 },
      }),
      vector_of(struct element, {
    ...
    input.three_dim = vector_of(struct vector, {
      vector_of(struct vector, {
        vector_of(byte, { 0, 1 }),
        vector_of(byte, { 16, 17 }),
      }),
      vector_of(struct vector, {
        vector_of(byte, { 32, 33 }),
        vector_of(byte, { 48, 49 }),
      }),
      vector_of(struct vector, {
        vector_of(byte, { 64, 65 }),
        vector_of(byte, { 80, 81 }),
      }),
    });
