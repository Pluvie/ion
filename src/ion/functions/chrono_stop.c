...
)
{
  ...
  c->stop = (li.QuadPart - UNIX_TIME_START) / TICKS_PER_SECOND;
  ...
  */
  ...
  c->stop = ts.tv_sec * 1000 + ts.tv_nsec / 1000000;

#else
  #error "⚡️ION⚡️: This platform does not implement the `chrono_stop` function."
#endif
}
