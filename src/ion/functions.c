#include "functions/allocator_init.c"
#include "functions/allocator_pop.c"
#include "functions/allocator_position_address.c"
#include "functions/allocator_position_get.c"
#include "functions/allocator_position_set.c"
#include "functions/allocator_push.c"
#include "functions/allocator_release.c"
#include "functions/allocator_shrink.c"
#include "functions/allocator_stretch.c"
#include "functions/bit_count_leading_ones.c"
#include "functions/bit_count_leading_zeros.c"
#include "functions/cstr_compare.c"
#include "functions/cstr_equal.c"
#include "functions/fail.c"
#include "functions/fatal.c"
#include "functions/hash_djb2.c"
#include "functions/hash_djb2_cstr.c"
#include "functions/memory_acquire.c"
#include "functions/memory_acquire_aligned.c"
#include "functions/memory_compare.c"
#include "functions/memory_copy.c"
#include "functions/memory_equal.c"
#include "functions/memory_move.c"
#include "functions/memory_release.c"
#include "functions/memory_release_aligned.c"
#include "functions/memory_resize.c"
#include "functions/memory_set.c"
#include "functions/print.c"
#include "functions/print_failure.c"
#include "functions/print_hex.c"
#include "functions/printl.c"
#include "functions/str_append.c"
#include "functions/str_compare.c"
#include "functions/str_empty.c"
#include "functions/str_equal.c"
#include "functions/str_strip.c"
#include "functions/str_to_dec.c"
#include "functions/str_to_int.c"
#include "functions/succeed.c"
