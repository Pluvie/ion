...
spec( memory_resize ) {
  ...
  when("the amount is zero") {
    ...
    must("fatally fail with a specific message");
      verify(cstr_equal(sim.fatal, "memory_resize: amount must not be zero"));
  ...
  when("the amount is greater than zero") {
    ...
    when("the realloc function fails") {
      ...
      must("fatally fail with a specific message");
        verify(cstr_equal(sim.fatal, "memory_resize: not enough memory"));
    ...
    when("the realloc function succeeds") {
      ...
      apply(preconditions);
      result = memory_resize(address, amount);
