...
spec( arena_push ) {
  ...
  #define preconditions \
    initialized_allocator = arena_create(32); \
  ...
  when("the allocator has no previous data") {
    when("the amount is not greater than the allocator capacity") {
      amount = 16;
      apply(preconditions);
      result = arena_push(allocator, amount);
    ...
    when("the amount is greater than the allocator capacity") {
      ...
      must("increase the allocator capacity to the amount");
