...
spec( memory_acquire ) {
  ...
  when("the amount is zero") {
    ...
    must("fatally fail with a specific message");
      verify(cstr_equal(sim.fatal, "memory_acquire: amount must not be zero"));
  ...
  when("the amount is greater than zero") {
    ...
    when("the malloc function fails") {
      ...
      must("fatally fail with a specific message");
        verify(cstr_equal(sim.fatal, "memory_acquire: not enough memory"));
